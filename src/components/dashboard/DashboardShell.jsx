      ...
      <div className="mx-auto flex min-h-screen max-w-[1600px]">
        <aside className="hidden w-72 shrink-0 border-r border-white/10 bg-slate-900/40 p-6 lg:flex lg:flex-col">
          ...
          <div className="mt-8 rounded-2xl border border-cyan-400/15 bg-cyan-400/5 p-4">
            <p className="font-medium">{marketplaceUser?.name}</p>
            <p className="mt-1 text-sm text-slate-400">{marketplaceUser?.email}</p>
            <div className="mt-3 flex gap-2"><span className="rounded-full bg-cyan-400/10 px-3 py-1 text-xs uppercase text-cyan-300">{marketplaceUser?.role}</span><span className="rounded-full bg-violet-400/10 px-3 py-1 text-xs uppercase text-violet-300">{marketplaceUser?.subscription || "free"}</span></div>
          </div>
          <nav className="mt-8 space-y-2">{navigation.map((item) => {
            const active = item.href === "/dashboard" ? pathname === item.href : pathname.startsWith(item.href);
            return <Link key={item.href} href={item.href} className={`block rounded-xl px-4 py-3 text-sm font-medium ${active ? "bg-cyan-400 text-slate-950" : "text-slate-300 hover:bg-white/5"}`}>{item.label}</Link>;
          })}</nav>
