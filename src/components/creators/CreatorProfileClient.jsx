...
import FollowCreatorButton from "@/components/creators/FollowCreatorButton";
...
export default function CreatorProfileClient() {
        ...
        if (active) {
          setError(
            requestError.message ||
              "Unable to load the creator profile.",
          );
        }
      } finally {
        if (active) {
  ...
  function updateFollowerCount(difference) {
    setCreator((current) => {
      if (!current) {
        return current;
      }
  ...
  async function saveProfile(event) {
    ...
    try {
      ...
      if (
        typeof refreshMarketplaceSession ===
        "function"
      ) {
      ...
      toast.error(
        requestError.message ||
          "Unable to update the creator profile.",
      );
    } finally {
      setSaving(false);
    }
          ...
          <div className="flex flex-col gap-7 md:flex-row md:items-center">
            ...
            <div className="flex-1">
              ...
              <p className="mt-3 max-w-3xl leading-7 text-slate-300">
                {creator.bio ||
                  "This creator has not added a public biography yet."}
              </p>
          ...
          >
            ...
            <label className="sm:col-span-2">
              ...
              <input
                ...
                onChange={updateField}
                placeholder="Marketing, Writing, Development"
                className="mt-2 w-full rounded-xl border border-slate-300 px-4 py-3"
              />
            </label>
        ...
        <div className="mt-12">
          ...
          {prompts.length === 0 ? (
            <div className="mt-7 rounded-3xl border border-dashed border-slate-300 p-10 text-center text-slate-600">
              This creator has no approved
              marketplace prompts yet.
            </div>
          ) : (
